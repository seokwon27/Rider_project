...
const pathInformationUrl = import.meta.env.VITE_PATH_INFORMATION_URL;
...
export const getSearchRoad = async (searchValue, selectedButton) => {
  ...
  if (selectedButton === "전체") {
    ...
    const response = await axios.get(
      `${roadInfoUrl}/amenities?Classification=${selectedButton}&name_like=${searchValue}`
    );
    return response.data;
  }
};
...
export const getAmenities = async (id) => {
  const response = await axios.get(`${pathInformationUrl}/amenities?id=${id}`);
  return response;
};
